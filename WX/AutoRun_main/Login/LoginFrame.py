...
class LoginFrame(wx.Frame):
    ...
    def login_panel_init(self):
        ...
        self.vbox_main.Add(wx.StaticLine(self.login_panel), 0, wx.ALL | wx.EXPAND, 0)

        # USER
        hbox1 = wx.BoxSizer(wx.HORIZONTAL)
        self.sText1 = wx.StaticText(self.login_panel, label='User name:', size=(80, 20))
        ...
        fgs.AddMany([(self.sText1, 1, wx.ALIGN_BOTTOM), (self.tCtrl_user, 1, wx.EXPAND)])
        fgs.AddGrowableCol(1, 1)
        hbox1.Add(fgs, proportion=1, flag=wx.ALL | wx.EXPAND, border=2)
        self.vbox_main.Add(hbox1, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, border=2)
        self.vbox_main.Add((-1, 5))

        # PASSWORD
        hbox2 = wx.BoxSizer(wx.HORIZONTAL)
        self.sText2 = wx.StaticText(self.login_panel, label='Password:', size=(80, 20))
        ...
        self.tCtrl_password = wx.TextCtrl(self.login_panel, style=wx.TE_PASSWORD |
                                                                  wx.TE_PROCESS_ENTER)
        self.tCtrl_password_show = wx.TextCtrl(self.login_panel, style=wx.TE_PROCESS_ENTER)
        ...
        self.btn_password = wx.Button(self.login_panel, label='..', size=(30, 20))
        # fgs2 = wx.FlexGridSizer(1, 2, 0, 0)  # FlexGridSizer(rows, cols, vgap, hgap)
        fgs2 = wx.FlexGridSizer(1, 3, 0, 0)  # FlexGridSizer(rows, cols, vgap, hgap)
        fgs2.AddMany([(self.sText2, 1, wx.ALIGN_BOTTOM),
                      (self._password_sizer, 1, wx.EXPAND),
                      (self.btn_password, 1, wx.EXPAND)])
        fgs2.AddGrowableCol(1, 1)  # Specifies that column idx (starting from zero) should be grown
        # if there is extra space available to the sizer.
        hbox2.Add(fgs2, proportion=1, flag=wx.ALL | wx.EXPAND, border=2)
        self.vbox_main.Add(hbox2, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, border=2)
        # self.vbox_main.Add((-1, 5))

        # CONFIRM PASSWORD
        self.hbox_pass_confirm = wx.BoxSizer(wx.HORIZONTAL)
        self.sText_pass_cf = wx.StaticText(self.login_panel, label='Confirm:', size=(80, 20))
        # self.sText2.SetFont(font)
        self.tctrl_pass_cf = wx.TextCtrl(self.login_panel, style=wx.TE_PASSWORD)
        self.sText_pass_cf.Hide()
        self.tctrl_pass_cf.Hide()
        self.fgs_pass_cf = wx.FlexGridSizer(1, 2, 0, 0)  # FlexGridSizer(rows, cols, vgap, hgap)
        self.fgs_pass_cf.AddMany([(self.sText_pass_cf, 1, wx.ALIGN_BOTTOM),
                                  (self.tctrl_pass_cf, 1, wx.EXPAND)])
        self.fgs_pass_cf.AddGrowableCol(1, 1)  # Specifies that column idx (starting from zero)
        # should be grown
        # if there is extra space available to the sizer.
        ...
        self.sText_email = wx.StaticText(self.login_panel, label='Email:', size=(80, 20))
        # self.sText2.SetFont(font)
        self.tctrl_email = wx.TextCtrl(self.login_panel)
        self.sText_email.Hide()
        self.tctrl_email.Hide()
        self.fgs_email = wx.FlexGridSizer(1, 2, 0, 0)  # FlexGridSizer(rows, cols, vgap, hgap)
        self.fgs_email.AddMany([(self.sText_email, 1, wx.ALIGN_BOTTOM),
                                (self.tctrl_email, 1, wx.EXPAND)])
        self.fgs_email.AddGrowableCol(1, 1)  # Specifies that column idx (starting from zero)
        # should be grown
        # if there is extra space available to the sizer.
        self.hbox_email.Add(self.fgs_email, proportion=1, flag=wx.ALL | wx.EXPAND, border=2)
        self.vbox_main.Add(self.hbox_email, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, border=2)

        # REMEMBER PASSWORD
        hbox4 = wx.BoxSizer(wx.HORIZONTAL)
        self.sText4_empty = wx.StaticText(self.login_panel, label='', size=(80, 20))
        self.checkbox4 = wx.CheckBox(self.login_panel, label='Remember my password')
        self.checkbox4.Show()
        ...
        self.vbox_main.Add(hbox4, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, border=2)
        self.vbox_main.Add((-1, 5))
        ...
        self.btn2.Disable()
        self.btn3 = wx.Button(self.login_panel, label='SIGN UP', size=(140, 30))
        self.sText6_empty = wx.StaticText(self.login_panel, label='', size=(80, 20))
        # gs = wx.GridSizer(1, 3, 5, 5) # 1 x 3
        fgs6 = wx.FlexGridSizer(1, 3, 0, 0)  # FlexGridSizer(rows, cols, vgap, hgap)
        ...
        # INFO
        self.text_info = wx.StaticText(self.login_panel, label='', size=(200, 20))
        ...
        self.btn_help = wx.Button(self.login_panel, label="I CAN'T SIGN IN...", size=(250, 30))
        ...
        self.vbox_main.Add(hbox7, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, border=2)
        self.vbox_main.Add((-1, 5))
        ...
        self.Bind(wx.EVT_TEXT, self.tctrl_password_show_event, self.tCtrl_password_show)
    ...
    def btn2_event(self, evt):
        ...
        if not self._isCreateAccount:
            ...
            if rsp is 1:
                ...
                self.text_info.SetLabel("Something wrong, check your connection")
        ...
        # in case create new Account
            ...
            if not self.isAdmin:
                self.text_info.SetLabel("")
                self.text_info.SetLabel("Contact to tdchung to create new account")
            else:
    ...
    def btn3_event(self, evt):
        ...
        self.SetTitle("Create new account.." if self._isCreateAccount else "Login form")
        self.tctrl_pass_cf.GetParent().Layout()
        myPrint.debug("DEBUG: self._isCreateAccount: %s" % str(self._isCreateAccount))
